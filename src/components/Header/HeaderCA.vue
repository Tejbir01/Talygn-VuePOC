            ...
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item dropdown position-unset">
                    <a class="nav-link dropdown-toggle sidebaricon position-relative"
                       ...
                       aria-haspopup="true"
                       aria-expanded="false">
                        <i class="fa fa-th-large i-lmenu"></i><span>Full Menu</span>
                    </a>
                    <div v-bind:class="{'custom-width-menu':TotalCount<=5}" class="dropdown-menu dropdown-menu-right full-menu-new overflow-visible mt-0" id="fullMenuUl" aria-labelledby="navbarDropdownPortfolio" style="max-height:1000px;">
                        <div class="menu-search-panel">
                            <input type="text" name="search" value="" class="menusearchitem" placeholder="TypetoSearchMenuitems" />
                        ...
                        <div class="navigation-panel w-100">
                            <div class="nav-heading" style="display:none;">
                                ...
                                <span class="back ml-auto"><a class="cursor-pointer ancbacktomenu" title="@Resources.Back"><i class="fa fa-chevron-left"></i>@Resources.Back</a></span>
                            </div>
                            <div id="scrollbar" style="max-height:290px!important;">
                                <ul v-for="parent in MenuDataGroupBy">
                                    <li v-for="item in parent.Menu">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li>
                                    <a class="dropdown-item sidebarCollapse"
                                       ...
                                       valuefor="L"
                                       data-toggle-tooltip="tooltip"
                                       data-original-title="Left">Left</a>
                                </li>
                            </ul>
                        </li>
                        <li>
...
<script>
    ...
    $(".ancbacktomenu").click(function (event) {
        event.stopImmediatePropagation();
    ...
    function SearchMenuItems(val) {
        if ($("ul.submenu:visible").length > 0 && $("#hdnischildsearch").val() == "1") {
        ...
        else { // $("#fullMenuUl").find("ul:not(.submenu)").find("li.menuitem").find("a").each(function () {
            //Search parent menu item
            $("#fullMenuUl").find("ul:not(.submenu)").find("li.menuitem").find("a").each(function () {
                val = String(val).toLowerCase();
                var text = String($(this).text()).toLowerCase();
                if (text.includes(val)) {
                    $(this).closest("li.menuitem").show();
            ...
            //Search child menu item
            $("#fullMenuUl ul.submenu").find("li.menuitem a").each(function () {
                val = String(val).toLowerCase();
                var text = String($(this).text()).toLowerCase();
                if (text.includes(val)) {
                    $(this).closest("li.menuitem").show();
                    var pid = $(this).closest("ul.submenu").attr("data-p-id");
                    if ($("li[data-id='" + pid + "']").length > 0) {
                        $("li[data-id='" + pid + "']").show();
                        $("div.navigation-panel").find('.hdrGroupName').addClass('d-none').removeClass("d-flex");
                        $("div.navigation-panel").find('.nav-heading').show().find("#spntext").html('<span><i class="fa fa-th-list d-none"></i>' + $(this).closest("ul.submenu").attr("data-p-name") + '</span>');                        
                        ShowChild($("li[data-id='" + pid + "']"), true);
                    }
                }
                else {
                    $(this).closest("li.menuitem").hide();
    ...
    $("input.menusearchitem").keyup(function (event) {
        event.stopImmediatePropagation();
        var val = $(this).val();
        if (val != "") {
            $("a.clear-search").show();
