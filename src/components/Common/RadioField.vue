...
<template>
    <Field :name="name"  :rules="{'required':isRequired?true:false}" :label="name" v-slot="{ field, errorMessage, meta}" type="radio">       
        <label v-for="item in checkOptions" :key="item">
            <input type="radio" :name="item.name" v-bind="field" :value="item.value" />item.name
            </label>
